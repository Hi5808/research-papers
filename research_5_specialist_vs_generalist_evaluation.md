# Does Domain-Specialist Fine-Tuning Beat a Generalist? Evaluation Depth, Training Data Depth, and a Reversal
...
A common assumption in small-model fine-tuning is that narrowing a model's training data to a single domain will outperform a generalist model of the same size on in-domain tasks. Testing this directly across four IT-operations diagnostic domains (Windows, Linux, networking, hardware) at 1.5B parameters required three successive rounds of methodology correction before a trustworthy, interpretable result emerged. Round 1 (shallow, single-hop test questions) found no detectable difference. Round 2 (deeper, multi-hop test questions) revealed a real but domain-dependent effect — the specialist won clearly on Windows, the generalist won clearly on networking. Round 3 tested whether that domain-dependence was actually a *training data depth* confound rather than a property of the domain itself: adding 15 deep, multi-hop training scenarios per domain (disjoint from the held-out eval set) reversed networking's result entirely — from a clear generalist win (70% pass rate) to a clear specialist win (85%) — while producing a smaller improvement in hardware and a slight regression in linux. The final, most defensible conclusion is narrower than either the "specialists never help" or "specialists always help" story: specialization's benefit depends heavily on whether the training data itself contains examples of the deep reasoning the model is later asked to perform, and domain identity alone is not the deciding factor.
...
**Implication:** For narrow domains with less data or thinner topical documentation available to the synthetic-generation teachers, training data composition — specifically, whether the data contains examples of the *kind* of reasoning depth you intend to evaluate — appears to matter more than domain identity itself. A domain that looks like it "doesn't benefit from specialization" may actually be under-resourced in training data depth rather than fundamentally unsuited to specialization. This reframes round 2's finding: the earlier "domain-dependent effect" may be more accurately described as a "data-composition-dependent effect" that correlated with domain identity in this specific dataset, rather than being caused by domain identity.
...
This work set out to test whether narrow domain fine-tuning outperforms a generalist at small model scale (1.5B parameters, 2,000-4,000 training pairs per domain). The answer changed three times as both the evaluation methodology and the training data were interrogated and corrected: an initial exact-keyword test produced confidently-wrong "failing" scores on models that were actually correct; a corrected but shallow judge-based test (n=10, single-hop questions) found no detectable difference at all; a deeper judge-based test (n=20, multi-hop questions) revealed a real but domain-dependent effect — specialist wins on Windows, generalist wins on networking; and finally, adding deep multi-hop *training* scenarios (not just eval scenarios) reversed networking's result entirely, from the domain where specialization looked actively harmful to the domain with the strongest specialist advantage measured in this entire project.

None of the first three answers should be trusted in isolation — each was superseded by a more rigorous version of the same experiment. The final, most defensible conclusion: **specialization's benefit at this scale is not primarily a property of the domain, but of whether the training data actually contains examples of the reasoning depth the model will later be asked to perform.** Two domains (windows, network) that were trained closer to this standard produced measurable specialist advantages; linux, which received a smaller *proportional* addition of deep training data relative to its already-larger existing dataset, showed no improvement and possibly a small regression. This is consistent with — though not proof of — the idea that the earlier "domain-dependence" finding was substantially a training-data-composition confound rather than an intrinsic property of Windows vs. networking vs. hardware knowledge.

What this work establishes with confidence, independent of the specific domain results: (1) evaluation methodology for small-model comparisons requires real engineering investment before any A/B result should be trusted — three successive corrections were each necessary, not optional polish; (2) test *depth*, not just size, determines whether an evaluation can detect a real effect; and (3) the same principle applies to training data — a synthetic data pipeline that only generates shallow, single-hop examples will produce a model that performs like a generalist on deep problems, regardless of how narrow its topic scope was. Narrowing *topic* without also deepening *reasoning demand* in the training data may not produce genuine specialization at all.
